...
export function ProgressoProvider({ children }: { children: ReactNode }) {
  // Carrega o estado inicial do localStorage de forma idêntica ao seu script antigo[cite: 22]
  ...
  const desbloquearConteudo = (id: string, nomeExibicao: string) => {
    if (desbloqueados[id]) return; // Evita re-desbloqueio desnecessário[cite: 22]

    const novoEstado = { ...desbloqueados, [id]: true };
    setDesbloqueados(novoEstado);
    localStorage.setItem('fragmentos_eternidade_lore', JSON.stringify(novoEstado));[cite: 22]

    // Sistema de Pop-up reativo (Substitui o Lore.popup)[cite: 22]
      ...
      <AnimatePresence>
        {notificacao && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 glass-card border border-mythic-gold/30 rounded-xl bg-void-black/95 shadow-[0_0_30px_rgba(212,175,55,0.15)]"
          >
            ...
            <div>
              ...
              <p className="text-xs font-cinzel font-bold text-white tracking-wide">📜 {notificacao}</p>[cite: 22]
            </div>
          </motion.div>
        )}
      </AnimatePresence>
